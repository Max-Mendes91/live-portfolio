...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (!serviceData) {
    return {
      title: 'SEO & Performance Optimization | Max Mendes',
    };
  }
...
export default async function SeoPerformanceOptimizationPage() {
  const dictionary = await getDictionary('en');
  const serviceData = dictionary.servicePages?.[SERVICE_ID] as ServiceLink | undefined;
      ...
      <div className="min-h-screen bg-background text-text-primary">
        <div className="max-w-4xl mx-auto px-6 py-24">
          <h1 className="text-4xl font-light tracking-tight mb-6">
            {serviceData?.seo.h1 || 'SEO & Performance Optimization'}
          </h1>
