...
import { ServiceLink, HomePageDict, CaseStudyPageDict, BlogPageDict, BlogPostMeta } from '@/types/i18n';
...
// Generate metadata for case study pages from dictionary
export function generateCaseStudyPageMetadata(
  caseStudyData: CaseStudyPageDict,
  locale: SupportedLocale = 'en'
): Metadata {
  const { seo, hrefLang, href, schema, content } = caseStudyData;
  ...
  // Use ogDescription if available, otherwise use metaDescription
  const ogDescription = seo.ogDescription || seo.metaDescription;

  // Get the hero image for Open Graph
  const ogImage = content.hero.image || '/og-image.png';
  ...
  return {
    ...
    openGraph: {
      ...
      modifiedTime: schema.dateModified || schema.datePublished,
...
// Generate metadata for blog listing page from dictionary
export function generateBlogListingMetadata(
  blogData: BlogPageDict,
  locale: SupportedLocale = 'en'
): Metadata {
  const { seo, hrefLang, href } = blogData;
...
// Generate metadata for individual blog post pages from MDX metadata
export function generateBlogPostMetadata(
  ...
  const languages: Record<string, string> = {};
  Object.entries(postMeta.hrefLang).forEach(([lang, path]) => {
    languages[lang] = getFullUrl(path);
  });
  languages['x-default'] = languages['en'];

  const ogImage = postMeta.image || '/og-image.png';

  return {
    ...generateBaseMetadata(locale),
    title: postMeta.title,
    description: postMeta.metaDescription,
    keywords: postMeta.keywords.join(', '),
    alternates: {
      canonical: canonicalUrl,
      languages,
    },
    openGraph: {
      title: postMeta.ogTitle,
      description: postMeta.metaDescription,
      url: canonicalUrl,
      siteName: SITE_CONFIG.name,
      locale: locale === 'en' ? 'en_US' : 'pl_PL',
      alternateLocale: locale === 'en' ? 'pl_PL' : 'en_US',
      type: 'article',
      publishedTime: postMeta.datePublished,
      modifiedTime: postMeta.dateModified || postMeta.datePublished,
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${postMeta.h1} - ${SITE_CONFIG.name}`,
        },
      ],
    },
    twitter: {
